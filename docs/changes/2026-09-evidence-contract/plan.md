...
- 范围：validation/tasks.py、任务契约模块、profiles.py、manage.py、相关 CLI 回归测试、根入口和相关维护文档、项目扩展规则模板、本记录与 TODO。
...
7. 可用性验证发现 local 文件编辑后重装会额外刷新锁并创建备份。用户要求修正；新增 CLI 回归先因 UPDATE 锁失败，保留已登记 local 指纹后通过。修复前候选及其原始检查输出保存在 Git 提交 14a7acac60368cb227606a3d8f09c31152295a91；当前证据替换为修复后候选的实测结果。
