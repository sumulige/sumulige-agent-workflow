...
小改动可引用已有 Issue；复杂任务先写方案，按[工作流程](workflow.md)明确目标、背景、路径与验收。方案写入仍须处于已授权范围。
阶段完成、关键决策、路径调整及交接时更新方案与相关工作文件，保留批准依据、实际改动、验证证据、未解决问题和下一步；引用现有事实来源，避免重复维护。
...
项目特有约定、当前事实入口及已采用资源索引写入 local 的 `.agent/project-rules.md` 或其明确引用的项目文件；共享入口按文件存在性读取。升级保留旧 local 内容，模板新增提示不会自动覆盖已安装项目。
根入口按任务触发读取；完整工程原则与资源引用正文在 reference/ 下独立维护，仍属于 core managed 分发。新增正文必须进入核心清单，安装、链接与缺失检查一起验收。
