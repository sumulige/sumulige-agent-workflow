...
[AGENTS.md](AGENTS.md) 保留工作约定与按需读取索引；十条[核心工程原则](docs/agent/reference/engineering.md)以用户结果与价值为起点，独立维护并随共享规则分发。Skills、MCP、仓库、知识库和 CLI 的定位与环境差异见[资源引用](docs/agent/reference/resources.md)。重要需求先填写[用户价值说明](docs/specs/_TEMPLATE/requirements.md#用户价值说明)，小改动可简写或引用已有记录。
